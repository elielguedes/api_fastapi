...
from schemas import usuario_schema , loguinschema
...
@auth_router.post("/loguin")
async def loguin(loguin_schema:loguinschema, session: Session = Depends(pegar_sessao)):
    usuario = session.query(Usuario).filter(Usuario.email == loguin_schema.email).first()
    if not usuario: #not e usado no if quando usuario já cadastrado ele já faz essa verifica
